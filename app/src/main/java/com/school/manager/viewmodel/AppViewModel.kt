...
class AppViewModel(app: Application) : AndroidViewModel(app) {
    ...
    fun lessonProgress(classId: Long): Pair<Int, Int> = state.value.lessons.progressFor(classId)

    // ─── Subjects ─────────────────────────────────────────────────────────────
    fun addSubject(name: String, color: Long, teacherId: Long?, code: String = "") {
        viewModelScope.launch { repo.addSubject(Subject(System.currentTimeMillis(), name, color, teacherId, code.ifBlank { genCode("SBJ") })) }
    }
    ...
    fun addTeacher(name: String, gender: String, phone: String = "", avatarUri: String? = null, code: String = "") {
        viewModelScope.launch { repo.addTeacher(Teacher(System.currentTimeMillis(), name, gender, phone, avatarUri, code.ifBlank { genCode("T") })) }
    }
    ...
    fun addSchoolClass(name: String, grade: String, count: Int, headTeacherId: Long?, subjectId: Long? = null, code: String = "") {
        viewModelScope.launch { repo.addSchoolClass(SchoolClass(System.currentTimeMillis(), name, grade, count, headTeacherId, subjectId, code.ifBlank { genCode("C") })) }
    }
    ...
    fun addStudent(name: String, studentNo: String, gender: String, grade: String, classIds: List<Long>, avatarUri: String? = null) {
        viewModelScope.launch { repo.addStudent(Student(System.currentTimeMillis(), name, studentNo, gender, grade, classIds, avatarUri)) }
    }
    ...
    fun batchGenerateLessons(
        ...
        viewModelScope.launch {
            val dates: List<LocalDate> = when (recurrenceType) {
                ...
                RecurrenceType.DAILY  -> buildList { var d = startDate; while (!d.isAfter(endDate)) { add(d); d = d.plusDays(1) } }
    ...
    // ─── KP Chapters ──────────────────────────────────────────────────────────
    fun addKpChapter(grade: String, no: Int, name: String) {
        viewModelScope.launch { repo.addKpChapter(KpChapter(System.currentTimeMillis(), grade, no, name)) }
    }
    fun updateKpChapter(c: KpChapter) { viewModelScope.launch { repo.updateKpChapter(c) } }
    fun deleteKpChapter(id: Long)      { viewModelScope.launch { repo.deleteKpChapter(id) } }

    // ─── KP Sections ──────────────────────────────────────────────────────────
    fun addKpSection(chapterId: Long, no: Int, name: String) {
        viewModelScope.launch { repo.addKpSection(KpSection(System.currentTimeMillis(), chapterId, no, name)) }
    }
    fun updateKpSection(s: KpSection) { viewModelScope.launch { repo.updateKpSection(s) } }
